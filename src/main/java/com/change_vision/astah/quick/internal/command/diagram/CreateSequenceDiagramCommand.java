...
public class CreateSequenceDiagramCommand implements CandidateSupportCommand {
    
    private CommittedNameTrimer trimer = new CommittedNameTrimer();

    @Immediate
    class SequenceDiagramCandidate implements Candidate {

        private final ElementCandidate owner;
        private final String name;

        public SequenceDiagramCandidate(ElementCandidate owner, String name) {
            this.owner = owner;
            this.name = name;
        }
        ...
        public String getDescription() {
            String description = format(Messages.getString("CreateSequenceDiagramCommand.SequenceDiagramCandidate.description"), owner.getName()); //$NON-NLS-1$
            return description;
        }
    ...
    @Override
    public String getName() {
        return "create sequence diagram"; //$NON-NLS-1$
    }

    @Override
    public String getDescription() {
        return Messages.getString("CreateSequenceDiagramCommand.description"); //$NON-NLS-1$
    }
    ...
    public Candidate[] candidate(Candidate[] committed, String searchKey) {
        ...
        if (name.isEmpty()) {
            return new Candidate[]{
                    new InvalidState(this,Messages.getString("CreateSequenceDiagramCommand.invalid_message")) //$NON-NLS-1$
            };
        }
        ...
        return new Candidate[]{
                new SequenceDiagramCandidate(elementCandidate,name)
        };
    }
    ...
    public void execute(Candidate[] candidates) throws ExecuteCommandException {
        if (candidates.length != 2) {
            throw new IllegalArgumentException(Messages.getString("CreateSequenceDiagramCommand.argument_length_error")); //$NON-NLS-1$
        }
        INamedElement owner;
        if ( (candidates[0] instanceof ElementCandidate) == false) {
            String message = format(Messages.getString("CreateSequenceDiagramCommand.candidate_error"),candidates[0]); //$NON-NLS-1$
        ...
        ISequenceDiagram diagram = api.createSequenceDiagram(owner,nameCandidate.getName());
        api.open(diagram);
    }
